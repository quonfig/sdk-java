#!/usr/bin/env bash
#
# Run the cross-SDK chaos harness against sdk-java (qfg-zqd0).
#
...
# 3. Runs `./gradlew :core:test --tests com.quonfig.sdk.chaos.ChaosTest` with CHAOS_RUN=1
#    (matches the artifact upload path in .github/workflows/chaos.yaml).
# 4. Tears everything down on exit (success or failure).
#
# Mirrors sdk-node/scripts/run-chaos.sh and sdk-go/scripts/run-chaos.sh so every
# SDK has identical boot semantics. Restored on the chaos-ci-wiring branch after
# the prior version (committed in qfg-47c2.5 / qfg-47c2.32) was not part of this
# branch's diff.
#
...
export QUONFIG_CHAOS_SESSION="${QUONFIG_CHAOS_SESSION:-sdk-java-$$-$(date +%s)}"
...
API_BIN="$SDK_JAVA_DIR/.chaos-api-delivery"
